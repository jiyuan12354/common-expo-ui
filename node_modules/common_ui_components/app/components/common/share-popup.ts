import { Clipboard, Platform } from 'react-native';
import Share from 'react-native-share';
import analytics, { firebase } from '@react-native-firebase/analytics';
import { GlobalHelper } from 'common_services_frontend/src/helpers/GlobalHelpers';
import { modalService as PruModalService } from '../../components/common/modal/ModalService';
import { getI18n } from 'react-i18next';
import { CommonTranslateKey } from '../../translation/__translation-keys';
import AsyncStorage from '@react-native-community/async-storage';
import { sha256 } from 'js-sha256';
import {
  AppRoutes,
  constants,
  LogActionType,
  LogEventType,
  LoggerHelper,
  LogRemark,
  LogSourceType,
} from 'common_services_frontend';
import { CONTENT_TYPE } from 'common_services_frontend/src/common-logic/portal-resource';
import RNFetchBlob from 'rn-fetch-blob';
// import { useSelector } from 'react-redux';

export interface ShareLinkOpt {
  title: string;
  url: string;
  message?: string;
  icon?: string;
  subject?: string;
  log?: any;
}

export interface ShareTextOpt {
  title?: string;
  message?: string;
  icon?: string;
  subject?: string;
  log?: any;
}

export interface ShareFileOpt {
  title?: string;
  urls?: string[];
  url?: string;
  type?: string;
  message?: string;
  icon?: string;
  log?: any;
}
export interface SharePDFOpt {
  title?: string;
  fileUrl: string;
  fileName: string;
  type: string;
  message?: string;
}

export interface SharePDFOpt {
  title?: string;
  fileUrl: string;
  fileName: string;
  type: string;
  message?: string;
}

export default function SharePopup(opts: ShareLinkOpt) {
  if (GlobalHelper.getStaffLoginFlag() && GlobalHelper.getStaffSelectedAgentCode()) {
    PruModalService.showAlert(
      getI18n().t(CommonTranslateKey.staff_block_alert_title),
      getI18n().t(CommonTranslateKey.staff_block_alert_label),
    );
    return new Promise((resolve, reject) => reject());
  }
  const {
    url = 'https://www.selfmgmt.com/pac/assessment/?testtype=pop7&function=cac&ac=Y53SH61J2C75&programlang=cht',
    title = '英國保誠',
    message = '保誠保險有限公司',
    icon = 'data:image/png;base64,',
    subject = '',
    // agentCode = 'unknown',
  } = opts;

  return Share.open(
    Platform.select({
      ios: {
        activityItemSources: [
          {
            // For sharing text.
            placeholderItem: { type: 'text', content: icon },
            subject: {
              default: subject || title,
            },
            item: {
              default: { type: 'text', content: `${title}\n${message}\n` },
              message: { type: 'text', content: `${title}\n${message}\n` }, // Specify no text to share via Messages app.
              mail: { type: 'text', content: `${title}\n${message}\n` }, // Specify no text to share via Mail.
              copyToPasteBoard: { type: 'text', content: `${title}\n${message}\n` }, // Specify no text to share via PasteBoard.
            },
          },
          {
            // For using custom icon instead of default text icon at share preview when sharing with message.
            placeholderItem: {
              type: 'url',
              // 顯示在分享彈框的Icon
              content: url,
            },
            item: {
              default: {
                // 分享類型
                type: 'url',
                // 分享類型，只允許url?
                content: `${url}`,
              },
            },
            linkMetadata: {
              // 顯示在分享彈框的title
              title,
              url,
              originUrl: url,
              icon,
            },
          },
        ],
      },
      default: {
        type: 'url',
        title,
        subject: subject || title,
        message: `${title}\n${message}\n${url}`,
      },
    }),
  ).then((data) => {
    LoggerHelper.emit(
      {
        source: url,
        sourceType: opts?.log?.source_type || LogSourceType.LINK,
        actionType: LogActionType.SHARE,
        eventType: LogEventType.MATERIAL_SHARING,
      },
      {
        material_shared: title,
      },
    );

    LoggerHelper.emit(
      {
        source: `${opts?.log?.source_type || LogSourceType.LINK}`,
        sourceType: opts?.log?.source_type || LogSourceType.LINK,
        actionType: LogActionType.SHARE,
        remarks:
          getLogRemarkForShare(opts?.log?.source_type || LogSourceType.LINK) ||
          LogRemark.SHARE_STATISTICS,
      },
      {
        material_shared: title,
        share_to: data?.app || 'Unknown',
        ...(opts?.log || {}),
      },
    );
  });
}

export function ShareLinkForSingleWhatsApp(opts: ShareLinkOpt) {
  const {
    url = 'https://www.selfmgmt.com/pac/assessment/?testtype=pop7&function=cac&ac=Y53SH61J2C75&programlang=cht',
    title = '英國保誠',
    message = '保誠保險有限公司',
    icon = 'data:image/png;base64,',
    subject = '',
    // agentCode = 'unknown',
  } = opts;

  return Share.shareSingle(
    Platform.select({
      default: {
        type: 'url',
        title,
        subject: title,
        message: `${title}\n${message}\n`,
        url,
        social: Share.Social.WHATSAPP,
        whatsAppNumber: GlobalHelper.getCurrentCandidatePhone(),
      },
    }),
  ).then((data) => {
    LoggerHelper.emit(
      {
        // for previus log
        source: `${opts?.log?.source_type || LogSourceType.LINK}`,
        sourceType: opts?.log?.source_type || LogSourceType.LINK,
        actionType: LogActionType.SHARE,
        eventType: LogEventType.MATERIAL_SHARING,
      },
      {
        material_shared: title,
      },
    );

    const shareLogType = opts?.log?.source_type || LogSourceType.LINK;
    LoggerHelper.emit(
      {
        source: `${shareLogType}`,
        sourceType: shareLogType,
        actionType: LogActionType.SHARE,
        remarks: getLogRemarkForShare(shareLogType) || LogRemark.SHARE_STATISTICS,
      },
      {
        share_to: 'whatsapp',
        ...(opts?.log || {}),
      },
    );
  });
}

const getLogRemarkForShare = (shareType) => {
  switch (shareType) {
    case LogSourceType.INTERVIEW:
      return LogRemark.INVITE_TO_ATTEND_INTERVIEW;
  }
};

export function SharePopupForCandidate(opts: ShareLinkOpt) {
  if (GlobalHelper.getStaffLoginFlag() && GlobalHelper.getStaffSelectedAgentCode()) {
    PruModalService.showAlert(
      getI18n().t(CommonTranslateKey.staff_block_alert_title),
      getI18n().t(CommonTranslateKey.staff_block_alert_label),
    );
    return new Promise((resolve, reject) => reject());
  }
  const currentCandidatePhone = GlobalHelper.getCurrentCandidatePhone();
  if (currentCandidatePhone) {
    const modalIdKey = ['SharePopupForCandidate', sha256(GlobalHelper.getAgentCode())].join(
      constants.joinStr,
    );
    return AsyncStorage.getItem(modalIdKey).then((modalId) => {
      return new Promise((resolve, reject) => {
        if (modalId) {
          PruModalService.shareOpen({
            onPickWhatsApp: () => {
              Clipboard.setString(`${opts.title}\n${opts.message}\n${opts.url}`);
              ShareLinkForSingleWhatsApp(opts)
                .then(() => resolve(''))
                .catch(() => reject());
            },
            onPickMore: () => {
              SharePopup(opts)
                .then(() => resolve(''))
                .catch(() => reject());
            },
          });
        } else {
          PruModalService.shareOpen({
            onPickWhatsApp: () => {
              Clipboard.setString(`${opts.title}\n${opts.message}\n${opts.url}`);
              PruModalService.confirm({
                title: getI18n().t(CommonTranslateKey.share_url_content_copied),
                description: getI18n().t(CommonTranslateKey.share_url_content_copied_tip),
                confirmText: getI18n().t(CommonTranslateKey.open_whatsapp),
                cancelText: getI18n().t(CommonTranslateKey.cancel),
                modalId: modalIdKey as string,
                onConfirm: (modalId: string) => {
                  AsyncStorage.setItem(modalId, modalId);
                  ShareLinkForSingleWhatsApp(opts)
                    .then(() => resolve(''))
                    .catch(() => reject());
                },
                onCancel: () => { },
              });
            },
            onPickMore: () => {
              SharePopup(opts)
                .then(() => resolve(''))
                .catch(() => reject());
            },
          });
        }
      });
    });
  } else {
    return SharePopup(opts);
  }
}
export const ShareFiles = (opts: ShareFileOpt) => {
  if (GlobalHelper.getStaffLoginFlag() && GlobalHelper.getStaffSelectedAgentCode()) {
    PruModalService.showAlert(
      getI18n().t(CommonTranslateKey.staff_block_alert_title),
      getI18n().t(CommonTranslateKey.staff_block_alert_label),
    );
    return new Promise((resolve, reject) => reject());
  }
  firebase.analytics().logEvent('material_sharing', {
    material_shared: opts.title,
    agent_code: GlobalHelper.getAgentCode(),
    created_at: String(new Date()),
  });
  return Share.open({ ...opts }).then((data) => {
    let fileName = '';
    if (opts.url && opts.url.split('/').length) {
      fileName = opts.url?.split('/').pop();
    }
    let source = `${LogSourceType.FILE}: ${fileName}`;
    if (source.length > 100) {
      source = `${LogSourceType.FILE}: omit due to the filename is too long`;
    }
    LoggerHelper.emit(
      {
        source: source,
        sourceType: LogSourceType.FILE,
        actionType: LogActionType.SHARE,
        remarks: LogRemark.SHARE_STATISTICS,
      },
      {
        material_shared: opts.title,
        share_to: data?.app || 'Unknown',
        ...(opts?.log || {}),
      },
    );
    return data;
  });
};

export const shareText = (opts: ShareTextOpt) => {
  const {
    message = '',
    subject = '',
    title = '',
    icon = 'data:image/png;base64,',
  } = opts;

  firebase.analytics().logEvent('material_sharing', {
    material_shared: title,
    agent_code: GlobalHelper.getAgentCode(),
    created_at: String(new Date()),
  });
  // const options = Platform.select({
  //   ios: {
  //     activityItemSources: [
  //       {
  //         // For sharing text.
  //         placeholderItem: { type: 'text', content: message },
  //         item: {
  //           default: { type: 'text', content: message },
  //           message: { type: 'text', content: message }, // Specify no text to share via Messages app.
  //           mail: { type: 'text', content: message }, // Specify no text to share via Mail app.
  //         },
  //         linkMetadata: {
  //           // For showing app icon on share preview.
  //           title: message,
  //         },
  //       },
  //     ],
  //   },
  //   default: {
  //     type: 'text',
  //     subject: subject || message,
  //     message: message,
  //   },
  // });
  return Share.open({
    type: 'text',
    subject: subject || message,
    message: message,
  }).then((data) => {
    LoggerHelper.emit(
      {
        source: `${CONTENT_TYPE.Text}`,
        sourceType: LogSourceType.TEXT,
        actionType: LogActionType.SHARE,
        remarks: LogRemark.SHARE_STATISTICS,
      },
      {
        material_shared: opts.title,
        share_to: data?.app || 'Unknown',
        ...(opts?.log || {}),
      },
    );
  });
};

export const shareStandardLink = (opts: ShareLinkOpt) => {
  const {
    url = 'http://test.e-ink.top/portal/redirect/prudna-zhhk/?redirect=https://www.selfmgmt.com/pac/assessment/?testtype=pop7&function=cac&ac=Y53SH61J2C75&programlang=cht',
    title = 'PRUDNA評估問卷',
    message = '[英國保誠]招聘經理Mia邀請您完成PRUDNA測評',
    icon = 'data:image/png;base64,',
  } = opts;

  firebase.analytics().logEvent('material_sharing', {
    material_shared: title,
    agent_code: GlobalHelper.getAgentCode(),
    created_at: String(new Date()),
  });

  Share.open(
    Platform.select({
      ios: {
        activityItemSources: [
          {
            // For sharing text.
            placeholderItem: { type: 'text', content: url },
            subject: {
              default: title,
            },
            item: {
              default: { type: 'text', content: `${title}\n${message}\n` },
              message: { type: 'text', content: `${title}\n${message}\n` }, // Specify no text to share via Messages app.
              mail: { type: 'text', content: `${title}\n${message}\n` }, // Specify no text to share via Mail.
              copyToPasteBoard: { type: 'text', content: `${title}\n${message}\n` }, // Specify no text to share via PasteBoard.
            },
          },
          // {
          //   // For using custom icon instead of default text icon at share preview when sharing with message.
          //   placeholderItem: {
          //     type: 'url',
          //     // 顯示在分享彈框的Icon
          //     content: icon,
          //   },
          //   item: {
          //     default: {
          //       // 分享類型
          //       type: 'url',
          //       // 分享類型，只允許url?
          //       content: `${url}`
          //     }
          //   },
          //   linkMetadata: {
          //     // 顯示在分享彈框的title
          //     title,
          //   },
          // },
          {
            placeholderItem: {
              type: 'url',
              content: url,
            },
            item: {
              default: {
                type: 'url',
                content: `${url}`,
              },
            },
            linkMetadata: {
              // 鏈接render之前顯示在分享頭部彈框的title, url
              title,
              // icon,
              url,
              originalUrl: url,
            },
          },
        ],
      },
      default: {
        type: 'url',
        title,
        subject: title,
        message: `${title}\n${message}\n${url}`,
      },
    }),
  );
};

export const sharePDF = (opts: SharePDFOpt) => {
  if (Platform.OS === 'ios') {
    return sharePDFWithIOS(opts);
  } else {
    return sharePDFWithAndroid(opts);
  }

};

const RNFS = require('react-native-fs');
const sharePDFWithIOS = (opts: SharePDFOpt) => {
  const { fileUrl, type, fileName, title, message } = opts
  let options = {
    type: type,
    url: fileUrl,
    fileName: fileName,
    title: title,
    message: message
  };
  Share.open(options);
}

function sharePDFWithAndroid(opts: SharePDFOpt) {
  let { fileUrl, type, fileName, title = '', message = '' } = opts
  // let filePath: string = '';
  let dirs = RNFetchBlob.fs.dirs

  RNFetchBlob.fs.readFile(fileUrl, 'base64')
    .then(async base64Data => {
      base64Data = `data:${type};base64,` + base64Data;
      // console.log('Debug --- base64Data', base64Data);

      await Share.open(
        {
          url: base64Data,
          filename: fileName,
          title: title,
          message: message
        });
    });
}
